...
#         bestScore, bestMove = self.__negamax__(gameState, self.__maxDepth__, gameState.getPlayerToPlayIndex())   
...
#     def __negamax__(self, gameState: GameState, depth: int, playerIndex: int) -> tuple[int, Move]:
#         """
...
#             - playerToPlayIndex : The index of the current player.
...
#                 nextScore, _ = self.__negamax__(gameState, depth - 1, playerIndex)
#                 if(playerIndex == gameState.getPlayerToPlayIndex()) : nextScore = - nextScore
                
#                 gameState.undo(move)
                
#                 if bestScore == None or nextScore > bestScore:

#                     if(bestScore != None) : e += self.getMoveStrengthToAdd(bestScore)
